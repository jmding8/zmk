...
struct behavior_hold_tap_config {
    ...
    enum flavor flavor;
    bool retro_tap;
    int32_t hold_behavior_enabler_keys[ZMK_KEYMAP_LEN];
    int32_t hold_behavior_enabler_keys_len;
};
...
static bool
are_any_hold_behavior_enabler_keys_depressed(const struct behavior_hold_tap_config *hold_tap_config) {
    struct behavior_hold_tap_config config = *hold_tap_config;
    if (hold_tap_config->hold_behavior_enabler_keys_len == 0) {
        return true;
    }
    
    for (int i = 0; i < hold_tap_config->hold_behavior_enabler_keys_len; i++) {
        if (is_key_held[config.hold_behavior_enabler_keys[i]]) {
            return true;
        }
    }
    return false;
...
static void decide_balanced(struct active_hold_tap *hold_tap, enum decision_moment event) {
    ...
    case HT_OTHER_KEY_UP:
        if (are_any_hold_behavior_enabler_keys_depressed(hold_tap->config)) {
...
static int position_state_changed_listener(const zmk_event_t *eh) {
    ...
    is_key_held[ev->position] = ev->state;
...
#define KP_INST(n)                                                                                 \
    static struct behavior_hold_tap_config behavior_hold_tap_config_##n = {                        \
        ...
        .hold_behavior_enabler_keys = DT_INST_PROP(n, hold_behavior_enabler_keys),                 \
        .hold_behavior_enabler_keys_len = DT_INST_PROP_LEN(n, hold_behavior_enabler_keys),         \
    };                                                                                             \
